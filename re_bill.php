...
if (!isset($_POST['startid'])) {
        header('Location: re_startid_in.php?error=スタートＩＤが入力されていません');
        exit;      
    }
...
try {
    ...
    foreach ($records as $record) {
        //billsendがNULLのみ請求書発行
        if ($record['billsend'] !== NULL ) {
            continue; 
           }
        // Excelファイルの書き込
        ...
        if (empty($record['nyuukainennd'])) {
            ...
            $nendo = $record['nyuukainenndo'];
            
            //新たに正会員TBに「nyuukainendo」というカラムを追加.GoogleFormで入会希望年度をインプット
        }
