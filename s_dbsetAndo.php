...
while ($data = fgetcsv($f)) {
    ...
    // CSV列数チェック（安全対策）
    if (!isset($data[6])) {
        echo "CSVフォーマットが不正です（列数不足）<br>";
        exit();
    }

    // 初期化
    $tax = $withtax = $id1 = $id2 = $kaiinId = $billmeans = $receiptmeans = null;
    $kubun = $data[5] ?? null;
    $regularFee = 0;
    $supportFee = 0;
    $sw = 0;
    ...
    if (preg_match('/（1）/', $target) && preg_match('/（2）/', $target)) {
        $data[6] = "２回分まとめて応募";
    } elseif (preg_match('/（1）/', $target)) {
        $data[6] = "１回目のみ応募";
    } elseif (preg_match('/（2）/', $target)) {
        $data[6] = "２回目のみ応募";
    } else {
        echo "参加するセミナー回数が不明<br>";
        echo "処理を中断します<br>";
        echo "応募者名＝".$data[2]."<br>";
        echo "メールAD＝".$data[1]."<br>";
        exit();
    }

    // plural と CSVフォーマットの一致チェック
    if ($plural == 2) {
        if (preg_match('/郵送/', $data[6]) ||
            preg_match('/PDF/', $data[6]) ||
            preg_match('/不要/', $data[6])) {
            $sw = 1;
        }
    }
    ...
    if ($plural == 1) {
        if (preg_match('/のみ/', $data[6]) ||
            preg_match('/まとめて/', $data[6]) ||
            preg_match('/回目/', $data[6])) {
            $sw = 1;
        }
    }
...
//複数人一括応募処理
    if ($plural == 2) {
        if ($data[14] == "複数人同時応募") {  // 比較には '==' を使用 
            if ($id1 == 2 || $id1 == 3) {  // 論理ORには '||' を使用　$id1は正会員等の区分　2;賛助会員（枠内）　3:賛助会員（枠超過）
                $n = 15;
